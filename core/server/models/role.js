...
Role = leafBookshelf.Model.extend({
...

  {
    ...
    update : Promise.method(function(attrs) {
        ...
        return t.forge().where({id : attrs.id}).fetch(options).then(function(role) {

          attrs = _.extend(role.serialize(),attrs);
          ...
          return t.forge({id : attrs.id}).save(attrs,options).then(function(role) {
              return role;
          });
