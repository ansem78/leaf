...
Share = leafBookshelf.Model.extend({
...

  {
    ...
    update : Promise.method(function(attrs) {
        ...
        return t.forge().where({id : attrs.id}).fetch(options).then(function(share) {

          attrs = _.extend(share.serialize(),attrs);
          ...
          return t.forge({id : attrs.id}).save(attrs,options).then(function(share) {
              return share;
          });
